# Core Mutation Survivor Triage

Issue #134 tracks the first focused survivor-triage pass after the initial Core Stryker.NET mutation baseline from Issue #121.

The goal of this pass is not to chase a raw score. The goal is to strengthen tests around Core governance behavior that would matter if it changed: evaluator composition, decision outcomes, audit residue, and acknowledgment or responsibility-handshake flow.

## Scope reviewed

This triage pass focused on the Core areas named in the issue:

| Area | Behavior under review | Triage classification | Action in this pass |
| --- | --- | --- | --- |
| `Evaluation` | Deny-wins composition, warning aggregation, policy metadata propagation, cancellation checks, and whether all constraint results reach decision policy. | Intentional behavior that needed stronger test coverage. | Added mutation-focused evaluator tests for multiple denials, warning aggregation, continued evaluation after denial, decision-policy result visibility, and cancellation between constraints. |
| `Decisions` | Outcome factories, reason-code/message preservation, trace and policy metadata propagation, and read-only reason snapshots. | Missing or weak assertion coverage. | Added mutation-focused decision tests for non-allow factory trace fields, reason message preservation, and read-only snapshot behavior. |
| `Audit` | `FromDecision` and `FromConstraint` propagation of outcome, reason codes, actor data, trace data, policy data, metadata, timestamp normalization, and source-collection aliasing. | Missing or weak assertion coverage. | Added mutation-focused audit tests for full trace/policy propagation and metadata snapshot behavior. |
| `Handshakes` | `FromDecision` reason selection, trace and policy propagation, metadata normalization, and acknowledgment response identity boundaries. | Missing or weak assertion coverage. | Added mutation-focused handshake tests for first-reason selection, decision trace/policy propagation, metadata snapshot behavior, and responding-actor identity. |

## Addressed survivor themes

The following survivor groups were addressed with stronger behavior assertions:
...
## Equivalent or non-actionable mutants

No mutants were suppressed in this pass.

Candidate equivalent or non-actionable mutants should be confirmed against the generated Stryker report before suppression. In particular, avoid suppressing a mutant only because it is inconvenient. Suppression should be reserved for cases where the mutated code is provably behavior-equivalent at the public API boundary or where the line belongs to non-runtime documentation-only code.

## Acceptable gaps for follow-up

Remaining follow-up work should be driven by the next Stryker.NET report. Likely follow-up areas include:
...
- equivalent-mutant documentation if Stryker reports mutants that cannot be killed through public behavior assertions;
...
After generating the report, review the surviving mutants against this triage note and either strengthen tests or document confirmed equivalent/non-actionable survivors.
